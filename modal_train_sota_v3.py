#!/usr/bin/env python3
"""
OmniClaw Parameter Golf — SOTA v3: DIM=512 + WD=0.15
42M params, aggressive WD for compression, all SOTA techniques
Target: 4500 steps on H100, fit under 16MB
"""
import modal
import subprocess
import os
...
app = modal.App("parameter-golf-sota-v3")
...
class Trainer:
    ...
    def train(self):
        ...
        print(f"🐉 OmniClaw SOTA v3 (512d WD=0.15) — {gpu_count}×{self.gpu_name}", flush=True)
        print(f"   Config: 13L, DIM=512, MLP×4, 8Q/4KV, WD=0.15, 4500 steps", flush=True)
        ...
        # SOTA v3: DIM=512 + WD=0.15 for compression + 4500 steps
        env = {
            **os.environ,
            "DATA_PATH": data_dir,
            ...
            # Architecture — same as v1 but with aggressive WD
            "VOCAB_SIZE": "8192",
            "MODEL_DIM": "512",
            "NUM_LAYERS": "13",
            "NUM_HEADS": "8",
            ...
            "MUON_WEIGHT_DECAY": "0.15",    # 0.095 → 0.15 for better compression
            "EMBED_WEIGHT_DECAY": "0.15",    # 0.085 → 0.15 for better compression
            "ADAM_WEIGHT_DECAY": "0.10",     # 0.04 → 0.10 for better compression
            "LATE_QAT_THRESHOLD": "0.0",
            # Training — 4500 steps for full convergence
            "ITERATIONS": "4500",
        ...
        print("🚀 Starting SOTA v3 (512d WD=0.15) training...", flush=True)
        print(f"   Key: 13L×512d MLP4, 8Q/4KV, WD=0.15, 4500 steps, GPTQ cal, embed_bits=8", flush=True)
        start = time.time()
        ...
        for fname in ["final_model.int6.brotli.ptz", "final_model.int6.ptz", "final_model.int8.ptz", "final_model.pt"]:
            ...
            if os.path.exists(src):
                dst = os.path.join("/models", f"sota_v3_{fname}")
                shutil.copy2(src, dst)
                print(f"  Saved → /models/sota_v3_{fname} ({os.path.getsize(dst):,} bytes)", flush=True)
        MODEL_VOL.reload()
