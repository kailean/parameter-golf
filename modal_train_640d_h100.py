...
def train():
    ...
    print(f"🐉 OmniClaw 1×H100 — {gpu_count}×{torch.cuda.get_device_name(0)}", flush=True)
    print(f"   Config: 640d MLPx4, emb8, custom serializer, no byte shuffle", flush=True)

    repo_dir = "/workspace/parameter-golf"

    # Clone repo (fresh each time to get latest code)
    ...
    # Data is already cached in the Modal volume at /data/sp8192_cache/datasets/
    # Symlink the cached data into the repo's data directory
    ...
    if not os.path.exists(data_dir):
        if os.path.exists(cached_datasets):
    ...
    env = {
        **os.environ,
        ...
        # Model config: 640d MLPx4 with SP8192
        "VOCAB_SIZE": "8192",
        "MODEL_DIM": "640",
        "NUM_LAYERS": "11",
        "NUM_HEADS": "10",
        ...
        "ROPE_BASE": "10000.0",
        "QK_GAIN_INIT": "5.25",
        "BIGRAM_HASH_SIZE": "0",  # CRITICAL: no bigram hash with SP8192
        "USE_ORTHO_INIT": "1",
        "SMEAR_ENABLED": "1",
        "XSA_LAST_N": "4",
        "ROPE_DIMS": "16",
        ...
        "WARMUP_STEPS": "20",
        "WARMDOWN_ITERS": "700",  # 35% of 2000
        "VAL_LOSS_EVERY": "200",
        "TRAIN_LOG_EVERY": "50",
        "MAX_WALLCLOCK_SECONDS": "0",  # no wallclock limit — run all 2000 steps
        # QAT disabled (late_qat=0, no mid-run recompile)
    ...
    if os.path.exists(model_path):
        ...
        print("  ❌ No model file found!", flush=True)

    # Copy model artifacts to persistent volume
    import shutil
    ...
    if log_files:
        ...
        with open(latest_log) as f:
            for line in f:
                if "val_bpb" in line or "serialized_int6_brotli" in line:
