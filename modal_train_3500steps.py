#!/usr/bin/env python3
"""
OmniClaw Parameter Golf — 1×H100 640d 3500-step Training
Same config as 2000-step run but with 3500 steps for real convergence.
...
app = modal.App("parameter-golf-640d-3500")
...
class Trainer:
    ...
    def train(self):
        ...
        print(f"🐉 OmniClaw 1×H100 3500-step — {gpu_count}×{self.gpu_name}", flush=True)
        print(f"   Config: 640d MLPx4, emb8, custom serializer, 3500 steps, warmdown 1225", flush=True)
        ...
        # Environment — 3500 steps with proper warmdown
        env = {
            **os.environ,
            "DATA_PATH": data_dir,
            ...
            "EMBED_BITS": "8",
            # Training config — 3500 steps
            "ITERATIONS": "3500",
            "TRAIN_BATCH_TOKENS": "786432",
            "WARMUP_STEPS": "20",
            "WARMDOWN_ITERS": "1225",  # 35% of 3500
            "VAL_LOSS_EVERY": "350",   # eval every 350 steps (10 evals)
            "TRAIN_LOG_EVERY": "50",
            "MAX_WALLCLOCK_SECONDS": "0",  # no wallclock limit
            # QAT disabled
            ...
            "EMA_START_FRAC": "0.5",
            "USE_SWA": "0",
            # TTT disabled for baseline
            "TTT_ENABLED": "0",
            "SEED": "42",
        }

        print("🚀 Starting 3500-step training...", flush=True)
        start = time.time()
        ...
        if log_files:
            ...
            print(f"\n📋 Key results from log: {latest_log}", flush=True)
            with open(latest_log) as f:
                for line in f:
                    if any(k in line for k in ["val_bpb", "serialized_int6", "Total submission", "final_", "FITS"]):
