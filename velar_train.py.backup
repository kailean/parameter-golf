...
def train_model():
    ...
    # Phase 1: 512d MLP×3 + WD warmdown + ALL SOTA techniques
    env = {
        **os.environ,
        "DATA_PATH": data_dir,
        ...
        "BROTLI_QUALITY": "11",
        # NEW SOTA techniques
        "GRADUATED_QAT": "1",                     # BF16→int8→int6
        "GRADUATED_QAT_INT8_FRAC": "0.30",
        "GRADUATED_QAT_INT6_FRAC": "0.50",
        "ENTROPY_REG_WEIGHT": "0.001",             # CAT loss
        "ENTROPY_REG_WARMUP_FRAC": "0.50",
        "SELF_GEN_GPTQ_CALIBRATION": "1",          # Model generates own data
        # WD warmdown
    ...
    print("Starting training with ALL SOTA techniques...", flush=True)
    start = time.time()
    ...
    return {"best_bpb": best_bpb, "size_mb": total / 1e6 if os.path.exists(model_path) else None}
