#!/usr/bin/env python3
"""
OmniClaw Parameter Golf — SOTA v4: 512d MLP×3 + WD Warmdown Schedule
~30M params, guaranteed <16MB, WD warmdown prevents regression
...
app = modal.App("parameter-golf-sota-v4")
...
class Trainer:
    ...
    def train(self):
        ...
        print(f"🐉 OmniClaw SOTA v4 (512d MLP×3 + WD warmdown) — {gpu_count}×{self.gpu_name}", flush=True)
        print(f"   Config: 13L, DIM=512, MLP×3, 8Q/4KV, WD=0.15→0.095 warmdown, 4500 steps", flush=True)
        ...
        # SOTA v4: 512d MLP×3 + WD warmdown schedule
        env = {
            **os.environ,
            "DATA_PATH": data_dir,
            ...
            # Architecture — smaller MLP for 16MB guarantee
            "VOCAB_SIZE": "8192",
            "MODEL_DIM": "512",
            "NUM_LAYERS": "13",
            "NUM_HEADS": "8",
            "NUM_KV_HEADS": "4",
            "MLP_MULT": "3",               # MLP×3 (was ×4) — fewer params, fits 16MB
            "TIE_EMBEDDINGS": "1",
            ...
            "BROTLI_QUALITY": "11",
            # CRITICAL: WD warmdown schedule (decay from 0.15→0.095 during warmdown)
            "MUON_WEIGHT_DECAY": "0.15",
            "EMBED_WEIGHT_DECAY": "0.15",
            "ADAM_WEIGHT_DECAY": "0.10",
            "WARMDOWN_WD_FRAC": "0.37",     # NEW: decay WD by 37% during warmdown (0.15→0.095)
            "LATE_QAT_THRESHOLD": "0.0",
            # Training schedule — shorter warmdown to reduce regression risk
            "ITERATIONS": "4500",
        ...
        print("🚀 Starting SOTA v4 (512d MLP×3 + WD warmdown) training...", flush=True)
        print(f"   Key: 13L×512d MLP3, 8Q/4KV, WD=0.15→0.095, warmdown=0.50", flush=True)
        start = time.time()
        ...
        for fname in ["final_model.int6.brotli.ptz", "final_model.int6.ptz", "final_model.int8.ptz", "final_model.pt"]:
            ...
            if os.path.exists(src):
                dst = os.path.join("/models", f"sota_v4_{fname}")
                shutil.copy2(src, dst)
                print(f"  Saved → /models/sota_v4_{fname} ({os.path.getsize(dst):,} bytes)", flush=True)
        MODEL_VOL.reload()
        ...
        if log_files:
            ...
            with open(log_files[0]) as f:
                for line in f:
                    if any(k in line for k in ["val_bpb", "serialized_int6_brotli", "final_", "total_params", "ttt_val", "FITS", "best_val"]):
