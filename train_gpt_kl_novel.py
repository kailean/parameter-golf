...
class Hyperparameters:
    ...
    embed_bits       = int(os.environ.get("EMBED_BITS", 6))
    ...
    # NOVEL: LeakyReLU² activation slope (0.0 = ReLU², 0.5 = LeakyReLU²(0.5))
    leaky_relu_slope     = float(os.environ.get("LEAKY_RELU_SLOPE", "0.0"))
    # NOVEL: Memory Tokens — compress older context into learned memory tokens
    memory_tokens       = int(os.environ.get("MEMORY_TOKENS", "0"))  # 0 = disabled
    memory_compress_len = int(os.environ.get("MEMORY_COMPRESS_LEN", "64"))  # compress first N tokens into memory tokens
    # NOVEL: Early Exit — dynamic layer activation gate (skip MLP on confident tokens)
    early_exit           = bool(int(os.environ.get("EARLY_EXIT", "0")))
    # NOVEL: Quantization-Aware Gating — learned correction after int6 dequant
    quant_gate_dim       = int(os.environ.get("QUANT_GATE_DIM", "0"))  # 0 = disabled
...
def dequantize_state_dict_int6(obj: dict[str, object], quant_gates: dict[str, Tensor] | None = None) -> dict[str, Tensor]:
    """Dequantize int6 state dict. If quant_gates provided, apply low-rank corrections
    to block weights: W_corrected = W_dequant + gate_up @ gate_down"""
    out: dict[str, Tensor] = {}
    qmeta  = obj.get("qmeta", {})
    ...
    # NOVEL: Quantization-Aware Gating — apply low-rank corrections to dequantized weights
    if quant_gates:
        for block_idx in range(len(quant_gates) // 2):  # each block has _up and _down
            up_key = f"block{block_idx}_up"
            down_key = f"block{block_idx}_down"
            if up_key in quant_gates and down_key in quant_gates:
                gate_up = quant_gates[up_key]    # (D, gate_dim)
                gate_down = quant_gates[down_key]  # (gate_dim, D)
                correction = gate_up @ gate_down  # (D, D) — low-rank
                # Apply to MLP fc (up-projection) weight — the biggest quantization loss
                fc_key = f"blocks.{block_idx}.mlp.fc.weight"
                if fc_key in out and correction.shape == out[fc_key].shape:
                    out[fc_key] = out[fc_key] + correction.to(dtype=out[fc_key].dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, rope_offset: int = 0) -> Tensor:
        ...
        # NOVEL: rope_offset = number of memory token positions at start of sequence.
        # Memory tokens get position 0 (no rotation), real tokens keep original positions.
        # We compute RoPE for positions [0..seqlen) but memory tokens (first rope_offset)
        # already sit at position 0 naturally. Real tokens need offset correction so they
        # get positions [0..seqlen-rope_offset) instead of [rope_offset..seqlen).
        cos, sin = self.rotary(seqlen, x.device, q.dtype)
        if rope_offset > 0:
            # Memory tokens (first rope_offset positions): replace with position-0 cos/sin (no rotation)
            cos0 = cos[:, :, :1, :].expand(-1, -1, rope_offset, -1)  # position 0 for memory tokens
            sin0 = sin[:, :, :1, :].expand(-1, -1, rope_offset, -1)
            # Real tokens: use positions [0..seqlen-rope_offset) so they keep original indices
            cos_real = cos[:, :, rope_offset:, :]  # shift back: position rope_offset becomes 0... 
            # Actually we need positions 0..T-1 for real tokens. The rotary cache already has
            # positions 0..seqlen-1. Memory tokens use position 0, real tokens use 0..(T-mem-1).
            # Easiest: just recompute with the right positions for real tokens.
            real_len = seqlen - rope_offset
            t_real = torch.arange(real_len, device=x.device, dtype=self.rotary.inv_freq.dtype)
            freqs_real = torch.outer(t_real, self.rotary.inv_freq.to(x.device))
            cos_real = freqs_real.cos()[None, None, :, :].to(dtype=q.dtype)
            sin_real = freqs_real.sin()[None, None, :, :].to(dtype=q.dtype)
            cos = torch.cat([cos0, cos_real], dim=2)
            sin = torch.cat([sin0, sin_real], dim=2)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, leaky_slope: float = 0.5):
        ...
        self.proj._zero_init = True
        self.leaky_slope = leaky_slope  # NOVEL: LeakyReLU²(0.5) instead of ReLU²

    def forward(self, x: Tensor) -> Tensor:
        # NOVEL: LeakyReLU(slope)² — allows small negative signals through
        # Original: relu(x).square() = max(0, x)² — kills negatives entirely
        # New: leaky_relu(x, slope)² — passes slope*x for x<0, then squares
        # This improves gradient flow in deeper models
        x = self.fc(x)
        x = torch.where(x > 0, x.square(), (x * self.leaky_slope).square())
        return self.proj(x)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, rope_base: float, qk_gain_init: float,
                 ...
                 leaky_slope: float = 0.0, early_exit: bool = False):
        super().__init__()
        ...
        # NOVEL: Dynamic Layer Activation (Early Exit)
        self.early_exit = early_exit
        if early_exit:
            self.exit_gate = nn.Linear(dim, 1, bias=True)
            nn.init.zeros_(self.exit_gate.weight)
            nn.init.constant_(self.exit_gate.bias, 1.0)

    def forward(self, x: Tensor, x0: Tensor, parallel_residual: bool = False, rope_offset: int = 0) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x   = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        s   = self.ln_scale_factor
        if parallel_residual:
            # GPT-J / PaLM style: attention and MLP in parallel
            x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * self.attn(self.attn_norm(x) * s, rope_offset=rope_offset) \
                   + self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x) * s)
        else:
            # Standard sequential: attention then MLP
            x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * self.attn(self.attn_norm(x) * s, rope_offset=rope_offset)
            # NOVEL: Dynamic Layer Activation — skip MLP when model is confident
            if self.early_exit and self.training:
                gate = torch.sigmoid(self.exit_gate(self.mlp_norm(x) * s))  # (B, T, 1)
                mlp_out = self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x) * s)
                x = x + gate * mlp_out  # learned gating: 0 = skip MLP, 1 = full MLP
            else:
...
class GPT(nn.Module):
    def __init__(
        ...
        memory_tokens: int = 0,
        memory_compress_len: int = 256,
        early_exit: bool = False,
        quant_gate_dim: int = 0,
    ):
        super().__init__()
        ...
        if memory_tokens > 0:
            self.memory_compress = nn.Linear(memory_compress_len * model_dim, memory_tokens * model_dim, bias=True)
            self.memory_norm = RMSNorm()
            nn.init.xavier_uniform_(self.memory_compress.weight)
            nn.init.zeros_(self.memory_compress.bias)
        ...
        self.blocks             = nn.ModuleList([
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                  ...
                  leaky_slope=leaky_slope, early_exit=early_exit)
            for i in range(num_layers)
        ])
        # NOVEL: Quantization-Aware Gating — low-rank correction matrices applied after int6 dequant
        # These are tiny learned matrices that compensate for quantization error.
        # At dequant time, each weight W becomes W_dequant + gate_up @ gate_down
        # gate_up: (dim, quant_gate_dim), gate_down: (quant_gate_dim, dim) — very low rank
        self.quant_gate_dim = quant_gate_dim
        ...
        # NOVEL: Quantization-Aware Gating — create low-rank correction pairs per block
        # quant_gate_dim > 0 enables this feature (set via QUANT_GATE_DIM env var)
        # Each block gets a (D, gate_dim) up-proj and (gate_dim, D) down-proj
        # These are NOT used during training — only applied post-dequantization
        self.quant_gates = None
        if quant_gate_dim > 0:
            self.quant_gates = nn.ParameterDict()
            for i in range(num_layers):
                # Up projection: (D, gate_dim) — compresses to low rank
                self.quant_gates[f"block{i}_up"] = nn.Parameter(
                    torch.randn(model_dim, quant_gate_dim, dtype=torch.float32) * 0.01
                )
                # Down projection: (gate_dim, D) — expands back
                self.quant_gates[f"block{i}_down"] = nn.Parameter(
                    torch.randn(quant_gate_dim, model_dim, dtype=torch.float32) * 0.01
                )
            print(f"Quantization-Aware Gating enabled: {num_layers} blocks, gate_dim={quant_gate_dim}, "
                  f"extra params={num_layers * 2 * model_dim * quant_gate_dim:,}")
    ...
    def _add_memory_tokens(self, x: Tensor, input_ids: Tensor) -> Tensor:
        """NOVEL: Compress older context into learned memory tokens.
        Takes first compress_len tokens and compresses them into num_memory tokens,
        prepended to the sequence. This gives the model access to longer-range context
        within a fixed token budget."""
        B, T, D = x.shape
        compress_len = min(self.memory_compress.weight.shape[1] // D, T)
        if compress_len < 1:  # Not enough context to compress
            return x
        prefix = x[:, :compress_len]  # (B, compress_len, D)
        mem = self.memory_compress(prefix.reshape(B, -1))  # (B, num_memory * D)
        mem = self.memory_norm(mem.reshape(B, self.memory_tokens, D))
        return torch.cat([mem, x], dim=1)  # (B, T + num_memory, D)

    def _run_blocks(self, x: Tensor, x0: Tensor, rope_offset: int = 0) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, parallel_residual=par, rope_offset=rope_offset)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, parallel_residual=par, rope_offset=rope_offset)
            ...
            for _ in range(self.recurrence_loops - 1):
                for li in self.recurrence_layers:
                    if li < len(self.blocks):
                        ...
                        x = self.blocks[li](x, x0, parallel_residual=par, rope_offset=rope_offset)
        return x
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.smear is not None:
            x = self.smear(x)
        # NOVEL: Memory Tokens — compress older context into learned tokens
        if self.memory_tokens > 0:
            x = self._add_memory_tokens(x, input_ids)
        rope_offset = self.memory_tokens  # memory tokens get position 0 (no RoPE)
        x0 = x
        x = self._run_blocks(x, x0, rope_offset=rope_offset)
        # If memory tokens were prepended, strip them from output before loss
        if self.memory_tokens > 0:
            x = x[:, self.memory_tokens:, :]  # (B, T, D) — remove memory prefix
        ...
        # NOVEL: Quant-Aware Gating — tiny contribution during training so DDP sees params as used
        # The real application is post-dequantization correction (gate_up @ gate_down)
        if self.quant_gates is not None:
            # Sum of gate norms as regularization (keeps params in gradient graph)
            gate_reg = sum(g.pow(2).mean() for g in self.quant_gates.values()) * 1e-8
            logits = logits + gate_reg
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        x0 = x
        x = self._run_blocks(x, x0, rope_offset=rope_offset)
        # If memory tokens were prepended, strip them from logits
        if self.memory_tokens > 0:
            x = x[:, self.memory_tokens:, :]  # (B, T, D) — remove memory prefix
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        memory_compress_len=args.memory_compress_len,
        early_exit=args.early_exit,
        quant_gate_dim=args.quant_gate_dim,
    ).to(device).bfloat16()
    ...
    # NOVEL: Store quant gates in serialized state for roundtrip
    if hasattr(base_model, 'quant_gates') and base_model.quant_gate_dim > 0:
        quant_obj_int6["quant_gates"] = {k: v.detach().cpu() for k, v in base_model.quant_gates.items()}
        quant_obj_int6["quant_gate_dim"] = base_model.quant_gate_dim
        log0(f"Quant gates stored: {len(base_model.quant_gates)} params, gate_dim={base_model.quant_gate_dim}")
    ...
    quant_gates = quant_state.pop("quant_gates", None)
    base_model.load_state_dict(dequantize_state_dict_int6(quant_state, quant_gates=quant_gates), strict=True)
